...
For the training time, there is one [example](Training_time.py) of calculating this metric. The calculation is based on the seconds. This example uses the Random permutation cross-validator to split the indices. You may use a simple Split method to reduce the calculation time.
